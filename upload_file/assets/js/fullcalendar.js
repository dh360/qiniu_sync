    ...
    var dpComputableOptions = {
        ...
        defaultButtonText: function (dpOptions) {
            return {
                ...
                // submit: stripHtmlEntities(dpOptions.submitText),
    ...
    */
            ...
            dragListener = new DragListener(this.coordMap, {
                ...
                cellOver: function (cell) {
                    ...
                    if (newEnd.isSame(oldEnd)) {
                        newEnd = null;
                    } else if (!calendar.isEventRangeAllowed({
                            start: start,
                            end: newEnd
                        }, event)) {
                        newEnd = null;
        ...
        fgSegHtml: function (seg, disableResizing) {
            ...
            if (view.isMultiDayEvent(event)) { // if the event appears to span more than one day...
                ...
                if (seg.isStart || seg.isEnd) {
                    ...
                    startTimeText = this.getEventTimeText({
                        start: seg.start
                    });
                }
            } else {
    ...
    var View = fc.View = Class.extend({
        ...
        massageCurrentDate: function (date, direction) {
            if (this.intervalDuration <= moment.duration({
                    days: 1
        ...
        reportEventResize: function (event, newEnd, el, ev) {
            var calendar = this.calendar;
            var mutateResult = calendar.mutateEvent(event, {
                end: newEnd
            });
            var undoFunc = function () {
                mutateResult.undo();
    ...
    setDefaults({
        yearColumns: 2,
        fixedWeekCount: 6 // 5 rows per month minimum (else true or false)
    });
    ...
    fcViews.year = View.extend({
        ...
        */
        ...
        numberCellHtml: function (cell) {
            ...
            html = '' + '<td class="' + classes.join(' ') + '" data-date="' + date.format() + '">' + date.date() + '</td>';
            // if (fes && fes.length > 0) {
        ...
        rowToGridRow: function (row) {
            ...
            for (var i = this.firstMonth; i < this.lastMonth; i++) {
                cnt += this.rowsForMonth[i];
                if (row < cnt) {
                    return row - (cnt - this.rowsForMonth[i]);
                }
            }
            return -1;
        },
